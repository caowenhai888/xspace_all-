...
import { ChevronRight, ArrowRight, Play, ChevronLeft } from 'lucide-react';
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#F5F4F0] pt-24 md:pt-40 overflow-x-hidden">
      ...
      <section className="container-custom mb-16 mt-10 md:mb-32 text-center text-black">
        <div className="mb-12">
          ...
          <Link href="/contact" className="flex items-center justify-center gap-2 w-[177px] h-[42px] bg-white border border-[#3871FF] rounded-[48px] text-black font-semibold text-[18px] hover:bg-gray-50 transition-all mx-auto shadow-sm">
            {t.technology.bookDemo} <ArrowRight size={14} strokeWidth={2.5} />
          </Link>
        </div>

        <div className="relative w-full aspect-[16/9] md:aspect-[1140/641] rounded-[12px] md:rounded-[18px] overflow-hidden mb-4 shadow-2xl bg-black group">
          <video 
            ref={egoVideoRef}
            key={`ego-v-${activeEgoIndex}`} 
            src={egoVideos[activeEgoIndex]}
            className="w-full h-full object-cover"
            controls={isEgoPlaying} // 播放时才显示控制条
            playsInline
          ...
          {!isEgoPlaying && (
            <div 
              className="absolute inset-0 flex items-center justify-center cursor-pointer bg-black/10 hover:bg-black/20 transition-all z-10"
        ...
        {/* 预览图网格 */}
        <div className="flex overflow-x-auto gap-3 md:gap-6 px-4 md:px-0 mb-4 md:mb-12 py-2 no-scrollbar md:grid md:grid-cols-4">
          {egoVideos.map((src, i) => (
            <div 
              key={i} 
              onClick={() => setActiveEgoIndex(i)} 
              className={cn(
                "relative flex-shrink-0 w-32 sm:w-40 md:w-auto aspect-[4/3] rounded-[10px] md:rounded-[18px] overflow-hidden shadow-md cursor-pointer transition-all duration-300",
                activeEgoIndex === i ? "border-[3px] border-[#3871FF] scale-105 z-10" : "opacity-60 hover:opacity-100"
              )}
            >
              <video src={`${src}#t=0.1`} className="w-full h-full object-cover pointer-events-none" preload="metadata" muted />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#DAECED] py-16 md:py-24 mb-16 md:mb-32">
        <div className="container-custom text-center text-black">
          ...
          <p className="text-base md:text-[20px] font-normal mb-10 md:mb-16 max-w-5xl mx-auto px-4 text-gray-800">
             Expanding beyond physical constraints. Bridging Reality and Simulation.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-12 px-4 md:px-0 mb-10">
            {[0, 1].map((offset) => {
          ...
          <div className="flex justify-center items-center gap-6">
            <button onClick={() => setLuminisPage(prev => Math.max(0, prev - 1))} className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-white transition-colors">
              <ChevronLeft size={20} />
            </button>
            <div className="flex gap-2">
              {[0, 1, 2, 3, 4, 5].map(p => (
                <div key={p} onClick={() => setLuminisPage(p)} className={cn("w-2 h-2 rounded-full cursor-pointer transition-all", luminisPage === p ? "bg-[#3871FF] w-6" : "bg-black/20")} />
              ))}
            </div>
            <button onClick={() => setLuminisPage(prev => Math.min(5, prev + 1))} className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-white transition-colors">
      ...
      {/* Section 3: X-One */}
      <section className="container-custom pb-16 md:pb-32 text-center text-black">
        <h2 className="mb-6 md:mb-8 text-2xl md:text-[56px] font-bold leading-[1.1]">{t.technology.xone.title}</h2>
        <p className="text-base md:text-[24px] mb-10 md:mb-12 max-w-4xl mx-auto px-4 text-gray-800">{t.technology.xone.desc}</p>
        <div className="relative w-full aspect-[16/9] md:aspect-[1140/641] rounded-[12px] md:rounded-[18px] overflow-hidden shadow-2xl bg-black group">
          <video 
            ref={xoneVideoRef} 
